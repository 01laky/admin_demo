/**
 * First-paint shell and localization failure UI before React mounts (admin SPA).
 */
...
export function renderBootstrapError(root: HTMLElement, apiUrl: string, onRetry: () => void): void {
	root.innerHTML =
		...
		'<p>Check that the API is running at <code style="background:#f4f4f4;padding:0.15rem 0.35rem">' +
		escapeHtml(apiUrl) +
		'</code>, then retry.</p>' +
		'<p style="color:#666;font-size:0.9rem">Endpoint: <code>GET /api/localization/admin</code></p>' +
